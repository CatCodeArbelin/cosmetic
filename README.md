...
- Приём входящих сообщений клиентов в личные сообщения живого Telegram-аккаунта магазина (Telethon).
...
- Клиенты пишут в личные сообщения живого аккаунта (через Telethon).
...
2. Заполните обязательные значения:
   - Telegram bot/account (включая `TELEGRAM_PHONE`, `TELEGRAM_SESSION_NAME`)
   - OpenAI
   - PostgreSQL
   - Redis
   - app/debounce
...
## 7) Авторизация живого Telegram-аккаунта (Telethon)
...
- при успешном входе session-файл (`TELEGRAM_SESSION_NAME`) сохраняется в `/app/sessions`;
